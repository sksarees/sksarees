...
/* absolute product page URL — 🔗 pretty per-product page: /product/SK75279.html
   (generated by Admin → Catalog Feed → "Generate Product Pages"). Old
   ?id= links keep working too. */
function repoBase(){
  try{
    let dir = location.pathname.replace(/[^/]*$/, '');       /* ends with / */
    if (/\/product\/?$/.test(dir)) dir = dir.replace(/\/product\/?$/, '/');   /* strip product folder when on a pretty page */
    return dir;
  }catch(e){ return '/'; }
}
function productUrl(p){
  try{ return location.origin + repoBase() + 'product/' + encodeURIComponent(p.id) + '.html'; }catch(e){ return 'product.html?id=' + encodeURIComponent(p.id); }
}
/* the product id for the CURRENT page — pretty pages set window.__PRODUCT_ID,
   pretty URL paths carry it too, and old ?id= links still work */
function currentProductId(){
  try{
    if (window.__PRODUCT_ID) return String(window.__PRODUCT_ID).trim();
    const m = location.pathname.match(/\/([^/]+)\.html$/);
    if (m && /^SK\d+$/i.test(m[1])) return m[1];
  }catch(e){}

...
function orderCard(o){
  ...
  return '<div class="order-card">' +
    ...
    '<div class="oc-btns">' +
      ...
      '<button type="button" class="btn btn-ghost btn-sm" data-delorder="' + esc(o.id) + '" style="width:auto;color:var(--red)">🗑️ Delete Order</button>' +
    '</div></div>';
}
...
function renderCoupons(){
  ...
  document.getElementById('cpSave').addEventListener('click', () => {
    ...
    let value = +document.getElementById('cpValue').value;
    if (document.getElementById('cpType').value === 'percent') value = Math.min(5, value);
    ...
    saveCoupons(coupons);
    renderCoupons();
    toast('✅ Coupon ' + code + ' created (percent coupons max 5%)');
  });
  renderCouponList();
}
